...
Справка:
Игрок может создавать сколько угодно юнитов при помощи метода spawn_unit(класс создаваемого юнита)

Может заставить юнитов атаковать через метод '-', который есть в каждом классе.

Может направлять в атаку сразу много юнитов через метод selected_atack(выбранные юниты, выбранные враги) 
...
атака метеоритом у магов через '*'
...
Может получить информацию о конкретном юните через метод info_about_unit(юнит).

Может получить информацию о конкретном классе через метод info_about_class(класс).

Может получить информацию о количестве живых юнитов у себя через метод desposition().
